...
const Doubt = () => {
    ...
    const doubturl = `${process.env.NEXT_PUBLIC_SERVER_URI}/api/v1/modules/doubt/allstudentdoubt?solved=false`;
    const getData = async () => {
            ...
            <div className='flex flex-wrap gap-[68px]  p-2 mt-8 font-semibold '>
                {
                    data?.map(value => (
                        <div className='bg-white rounded-[10px] w-[377px] border-2 flex-grow ' >
                            <div className='flex '>
                                <div className='bg-gradient-to-r from-[#FFC81D] to-[#FADB39]  w-[211px] text-[12px] pl-2 p-1 pr-4' style={{ borderRadius: "8px 0px 20px 0px " }}>{value?.chapter}</div>
                                <div className=''></div>
